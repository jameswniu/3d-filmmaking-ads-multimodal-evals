...
Thirteen of the sixteen named gating thresholds in [`../probes/`](../probes/) are computed from a labelled pass exemplar and a labelled fail exemplar. [`../evals/derive.py`](../evals/derive.py) prints the split and holds each derived constant inside the interval its own labels imply. The other three were typed by hand and are named on every run.
