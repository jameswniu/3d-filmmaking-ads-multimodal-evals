...
Sixteen of the sixteen named gating thresholds in [`../probes/`](../probes/) are computed from a labelled pass exemplar and a labelled fail exemplar, and [`../evals/derive.py`](../evals/derive.py) holds every one inside the interval its own labels imply.
