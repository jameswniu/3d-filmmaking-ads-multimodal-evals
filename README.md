...
| 2 | **Identity drift.** Is it the same person thirty seconds later, and in tomorrow's take? Measured as ArcFace cosine similarity against a reference face. Vendors name long-horizon identity stability as their top open problem. | **Constrained, not measured.** 174 identity records and 279 approved looks pinned to one allowlist. Pinning avoids the drift rather than detecting it. |
...
**What this pipeline sidesteps rather than solves.** The field also lists profile and side views, hands and fingers, multi-person scenes, emotion control, and cross-language audio as open. This renders one frontal avatar, alone, in one language, so none of them are load-bearing here. That is scope, not a solution, and it is the reason the five above are the ones worth measuring.
